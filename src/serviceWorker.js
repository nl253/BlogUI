...
export function register(config) {
  if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
    ...
    window.addEventListener('load', () => {
      ...
      if (!Boolean(window.location.hostname === 'localhost' ||
          window.location.hostname === '[::1]' ||
          window.location.hostname.match(
              /^127(?:\.(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}$/))) return registerValidSW(
          swUrl, config);
...
function registerValidSW(swUrl, config) {
  return navigator.serviceWorker.register(swUrl)
      ...
      .catch(error => console.error('Error during service worker registration:',
          error));
}
      ...
      .then(response => {
        ...
        if (response.status === 404 ||
            (contentType != null && contentType.indexOf('javascript') ===
                -1)) navigator.serviceWorker.ready.then(
